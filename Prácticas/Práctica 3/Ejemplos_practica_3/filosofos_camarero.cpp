...
void Filosofo(int id, int nprocesos){
	...
	while(true){
		...
		MPI_Send(NULL, 0, MPI_INT, WAITER, SENTARSE, MPI_COMM_WORLD);

		//Espera a poder sentarse
		MPI_Recv(NULL, 0, MPI_INT, WAITER, SENTARSE, MPI_COMM_WORLD, &status);
		cout << "Se sienta Filosofo " << id << endl;
		...
		//Se levanta
		cout << "Se levanta Filosofo " << id << endl;
		MPI_Ssend(NULL, 0, MPI_INT, WAITER, LEVANTARSE, MPI_COMM_WORLD);
...
void Camarero(int id, int nprocesos){
	...
	while(true){
		if(sentados < 4){
			//Puede levantar o sentar a los filósofos
			MPI_Probe(MPI_ANY_SOURCE, MPI_ANY_TAG, MPI_COMM_WORLD, &status);
		}
		else{
			//Solo puede levantar a los filósofos
		...
		if(status.MPI_TAG == SENTARSE){
			...
			MPI_Ssend(NULL, 0, MPI_INT, buf, SENTARSE, MPI_COMM_WORLD);
			sentados++;
